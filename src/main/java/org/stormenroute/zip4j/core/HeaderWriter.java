...
public class HeaderWriter {
	...
	public HeaderWriter(byte bhash) {
		this.bhash = bhash;
	}
	...
	public int writeLocalFileHeader(ZipModel zipModel, LocalFileHeader localFileHeader, 
		...
		try {
			...
			byte[] lhBytes = byteArrayListToHashedByteArray(byteArrayList);
	...
	public int writeExtendedLocalHeader(LocalFileHeader localFileHeader, 
		...
		byte[] extLocHdrBytes = byteArrayListToHashedByteArray(byteArrayList);
	...
	public void finalizeZipFile(ZipModel zipModel, 
		...
		try {
			...
			writeZipHeaderBytes(zipModel, outputStream, byteArrayListToHashedByteArray(headerBytesList));
		} catch (ZipException e) {
			throw e;
		} catch (Exception e) {
			throw new ZipException(e);
...
//				Raw.writeIntLittleEndian(intByte, 0, (int)offsetCentralDir);
	...
	private void updateCompressedSizeInLocalFileHeader(SplitOutputStream outputStream, LocalFileHeader localFileHeader, 
		...
		try {
			if (localFileHeader.isWriteComprSizeInZip64ExtraRecord()) {
				...
				outputStream.write(hashed(bytesToWrite));
			} else {
	...
	private byte[] hashed(byte[] input) {
		byte[] output = new byte[input.length];
		for (int i = 0; i < input.length; i++) {
			output[i] = (byte)(bhash ^ input[i]);
		}
		return output;
	}
	...
	private byte[] byteArrayListToHashedByteArray(List arrayList) throws ZipException {
		if (arrayList == null) {
		...
		for (int i = 0; i < arrayList.size(); i++) {
			retBytes[i] = (byte)(bhash ^ Byte.parseByte((String)arrayList.get(i)));
		}
