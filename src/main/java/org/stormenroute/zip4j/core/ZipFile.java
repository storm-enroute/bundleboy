...
import org.stormenroute.zip4j.io.ZipInputStream;
import org.stormenroute.zip4j.model.FileHeader;
import org.stormenroute.zip4j.model.UnzipParameters;
import org.stormenroute.zip4j.model.ZipModel;
import org.stormenroute.zip4j.model.ZipParameters;
import org.stormenroute.zip4j.progress.ProgressMonitor;
import org.stormenroute.zip4j.unzip.Unzip;
import org.stormenroute.zip4j.util.ArchiveMaintainer;
import org.stormenroute.zip4j.util.InternalZipConstants;
import org.stormenroute.zip4j.util.Zip4jUtil;
import org.stormenroute.zip4j.zip.ZipEngine;
...
public class ZipFile {
	 ...
	 * @throws ZipException
	 */
	public ZipFile(String zipFile, String password) throws ZipException {
	 ...
	 * @throws ZipException
	 */
	public ZipFile(File zipFile, String password) throws ZipException {
		if (zipFile == null) {
		...
		if (password == null) this.bhash = (byte)0;
		else this.bhash = byteHash(password);
	}

	private byte byteHash(String s) {
		int h = 0;
		for (int i = 0; i < s.length(); i++) h += (s.charAt(i) * 31) ^ (s.length() - i);
		byte bh = (byte) h;
		if (bh == 0) return (byte) 0xda;
		else return bh;
	}
	...
	public void addFiles(ArrayList sourceFileList, ZipParameters parameters) throws ZipException {
		...
		ZipEngine zipEngine = new ZipEngine(zipModel, bhash);
	...
	private void readZipInfo() throws ZipException {
		...
		try {
			...
			if (zipModel == null) {
				...
				HeaderReader headerReader = new HeaderReader(raf, bhash);
	...
	public void extractAll(String destPath, 
		...
		Unzip unzip = new Unzip(zipModel, bhash);
	...
	public void removeFile(FileHeader fileHeader) throws ZipException {
		...
		archiveMaintainer.removeZipFile(zipModel, fileHeader, bhash, progressMonitor, runInThread);
	}
	...
	public void mergeSplitFiles(File outputZipFile) throws ZipException {
		...
		archiveMaintainer.mergeSplitZipFiles(zipModel, outputZipFile, bhash, progressMonitor, runInThread);
	}
	...
	public void setComment(String comment) throws ZipException {
		...
		ArchiveMaintainer archiveMaintainer = new ArchiveMaintainer();
		archiveMaintainer.setComment(zipModel, bhash, comment);
	}
