...
package org.stormenroute.zip4j.core;

import java.io.IOException;
import java.io.RandomAccessFile;
import java.util.ArrayList;

import org.stormenroute.zip4j.exception.ZipException;
import org.stormenroute.zip4j.exception.ZipExceptionConstants;
import org.stormenroute.zip4j.model.AESExtraDataRecord;
import org.stormenroute.zip4j.model.CentralDirectory;
import org.stormenroute.zip4j.model.DigitalSignature;
import org.stormenroute.zip4j.model.EndCentralDirRecord;
import org.stormenroute.zip4j.model.ExtraDataRecord;
import org.stormenroute.zip4j.model.FileHeader;
import org.stormenroute.zip4j.model.LocalFileHeader;
import org.stormenroute.zip4j.model.Zip64EndCentralDirLocator;
import org.stormenroute.zip4j.model.Zip64EndCentralDirRecord;
import org.stormenroute.zip4j.model.Zip64ExtendedInfo;
import org.stormenroute.zip4j.model.ZipModel;
import org.stormenroute.zip4j.util.InternalZipConstants;
import org.stormenroute.zip4j.util.Raw;
import org.stormenroute.zip4j.util.Zip4jConstants;
import org.stormenroute.zip4j.util.Zip4jUtil;
...
public class HeaderReader {
	 ...
	 * @param zip4jRaf
	 */
	public HeaderReader(RandomAccessFile zip4jRaf, byte bhash) {
		this.zip4jRaf = zip4jRaf;
		this.bhash = bhash;
		this.ihash = ((int)bhash << 24) | ((int)bhash << 16) | ((int)bhash << 8) | bhash;
	}
	...
	private EndCentralDirRecord readEndOfCentralDirectoryRecord() throws ZipException {
		...
		try {
			...
			do {
				zip4jRaf.seek(pos--);
				counter++;
			} while ((Raw.readHashedLeInt(zip4jRaf, ebs, bhash) != InternalZipConstants.ENDSIG) && counter <= 3000);
			...
			//number of this disk
			readHashedIntoBuff(zip4jRaf, shortBuff);
			...
			readHashedIntoBuff(zip4jRaf, intBuff);
			...
			if (commentLength > 0) {
				...
				readHashedIntoBuff(zip4jRaf, commentBuf);
	...
	private CentralDirectory readCentralDirectory() throws ZipException {
		...
		try {
			...
			for (int i = 0; i < centralDirEntryCount; i++) {
				FileHeader fileHeader = new FileHeader(bhash);
				...
				if (fileNameLength > 0) {
					...
					readHashedIntoBuff(zip4jRaf, fileNameBuf);
...
//				}
				...
				if (fileCommentLength > 0) {
					...
					readHashedIntoBuff(zip4jRaf, fileCommentBuf);
			...
			if (sizeOfData > 0) {
				...
				readHashedIntoBuff(zip4jRaf, sigDataBuf);
	...
	private ArrayList readExtraDataRecords(int extraFieldLength) throws ZipException {
		...
		readHashedIntoBuff(zip4jRaf, extraFieldBuf);
	...
	private Zip64EndCentralDirLocator readZip64EndCentralDirLocator() throws ZipException {
		...
		try {
			...
			readHashedIntoBuff(zip4jRaf, longBuff);
	...
	private Zip64EndCentralDirRecord readZip64EndCentralDirRec() throws ZipException {
		...
		try {
			...
			if (extDataSecSize > 0) {
				...
				readHashedIntoBuff(zip4jRaf, extDataSecRecBuf);
	...
	private void setFilePointerToReadZip64EndCentralDirLoc() throws ZipException {
		try {
			...
			do {
				zip4jRaf.seek(pos--);
			} while (Raw.readHashedLeInt(zip4jRaf, ebs, bhash) != InternalZipConstants.ENDSIG);
...
//								- ZipConstants.STD_DEC_HDR_SIZE);
	 ...
	 * @throws ZipException
	 */
	private byte[] readHashedIntoBuff(RandomAccessFile zip4jRaf, byte[] buf) throws ZipException {
		try {
			int r = zip4jRaf.read(buf, 0, buf.length);
			if (r != -1) {
				for (int i = 0; i < r; i++) {
					buf[i] ^= bhash;
				}
